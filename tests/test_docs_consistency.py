...
class DocsConsistencyTests(unittest.TestCase):
    ...
    def test_key_anti_regression_semantics_have_canonical_homes(self) -> None:
        ...
        self.assertIn("背景与主体同视频烘焙", skill)
        # 只有明确透明复用才用 chroma
        self.assertIn("透明形式叠加到页面自有、可更换的背景", skill)
        # 多段同脸与实际尾帧接力
        self.assertIn("同脸", qa)
    ...
    def test_motion_brief_does_not_duplicate_contract_fields(self) -> None:
        ...
        section = source.split("建立 Motion Brief", 1)[1].split("### 3.", 1)[0]
        ...
        for field in (
            ...
            self.assertNotIn(
                f"{field}:", brief, f"Motion Brief 复制了合同字段 {field}"
            )
    ...
    def test_prompting_owns_submission_and_prompt_sections(self) -> None:
        ...
        self.assertIn("绿幕段（仅 chroma 路线）", doc)
