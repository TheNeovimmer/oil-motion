...
Agent 会先锁定 Concept Contract：人数、风格、情绪、叙事形式、背景归属、交互输入和连续性要求逐项确认，不会擅自把“单人动漫”扩写成其他方案。背景归属决定生产路线：场景叙事（镜头、环境光、地面接触、背景连续性重要）默认让背景与角色在同一视频中生成；只有主体需要以透明形式复用到页面背景时才使用绿幕。之后 Agent 根据画面大小、动画长度、访问方式和设备预算自动选择资源形式，不要求使用者理解或决定技术方案。
