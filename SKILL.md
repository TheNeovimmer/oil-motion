...
13. `chroma-video` 的离线 QA 与 WebGL 必须使用编译清单中的同一色键算法和参数；最终 H.264 编码后重新抽帧检查，未通过时不得生成可交付清单。
...
7. 视频路线只能从最终 MP4 解码帧生成 Alpha 海报、残色报告和背景验收矩阵；禁止用编码前 PNG 代替最终验收。
...
9. 最终分析通过后，严格按自动选择结果打包：`alpha-atlas` 生成单张 Alpha 图集；`chroma-video` 生成全关键帧绿幕 MP4，并对编码后的代表帧执行硬门槛检查。不要同时交付两套主实现。
10. 两条路线都用 `interactive-motion.ts` 控制整数帧；视频路线另外使用 `chroma-video-renderer.ts` 实时抠色，并直接读取 `compile.json.runtime`，不得手填帧数、锚点或色键参数。
